...
void AFish::CalculateFlockNewMoveVector(float DeltaTime)
{
	...
	else
	{
		DestLocation = FVector(
			FMath::FRandRange(SpawnLocation.X-WanderRange, SpawnLocation.X+WanderRange), 
			FMath::FRandRange(SpawnLocation.Y-WanderRange, SpawnLocation.Y+WanderRange), 
			FMath::FRandRange(SpawnLocation.Z-WanderRange, SpawnLocation.Z+WanderRange)
		);
	}
